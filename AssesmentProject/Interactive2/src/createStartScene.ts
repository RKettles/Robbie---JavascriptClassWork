...
function createTerrain(scene: Scene) {
  ...
  largeGroundMat.diffuseTexture = new Texture(
    "./assets/enviroments/snow.png"
  );

  const largeGround = MeshBuilder.CreateGroundFromHeightMap(
    "largeGround",
    "./assets/enviroments/snowheightmap.png",
    {
      width: 150,
      height: 150,
      subdivisions: 20,
...
function createGround(scene: Scene) {
  ...
  groundMaterial.diffuseTexture = new Texture(
    "./assets/enviroments/snowground.webp"
  );
...
function createSnowMen(scene: Scene) {
  const spriteManagerSnowmen = new SpriteManager(
    "snowmenManager",
   "./assets/sprites/snowmen.png",
    2000,
    { width: 512, height: 1024 },
    scene
  );

  //We create snowmen at random positions
  for (let i = 0; i < 500; i++) {
    const snowmen: Sprite = new Sprite("snowmen", spriteManagerSnowmen);
    snowmen.position.x = Math.random() * -30;
    snowmen.position.z = Math.random() * 20 + 8;
    snowmen.position.y = 0.2;
  }

  for (let i = 0; i < 500; i++) {
    const snowmen = new Sprite("snowmen", spriteManagerSnowmen);
    snowmen.position.x = Math.random() * 25 + 7;
    snowmen.position.z = Math.random() * -35 + 8;
    snowmen.position.y = 0.2;
  }
  // nothing returned by this function
}
